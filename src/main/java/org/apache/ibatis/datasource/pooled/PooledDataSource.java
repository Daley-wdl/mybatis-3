...
public class PooledDataSource implements DataSource {
    ...
    private static final Log log = LogFactory.getLog(org.apache.ibatis.datasource.pooled.PooledDataSource.class);
     ...
     * which are applying for new {@link org.apache.ibatis.datasource.pooled.PooledConnection}
     *
    ...
    public void forceCloseAll() {
        synchronized (state) {
            ...
            for (int i = state.activeConnections.size(); i > 0; i--) {
                try {
                    // 设置为失效
                    org.apache.ibatis.datasource.pooled.PooledConnection conn = state.activeConnections.remove(i - 1);
                    conn.invalidate();
            ...
            for (int i = state.idleConnections.size(); i > 0; i--) {
                try {
                    // 设置为失效
                    org.apache.ibatis.datasource.pooled.PooledConnection conn = state.idleConnections.remove(i - 1);
                    conn.invalidate();
        ...
        synchronized (state) {
            ...
            if (conn.isValid()) { // 有效
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    ...
                    org.apache.ibatis.datasource.pooled.PooledConnection newConn = new org.apache.ibatis.datasource.pooled.PooledConnection(conn.getRealConnection(), this);
    ...
    private org.apache.ibatis.datasource.pooled.PooledConnection popConnection(String username, String password) throws SQLException {
        boolean countedWait = false; // 标记，获取连接时，是否进行了等待
        org.apache.ibatis.datasource.pooled.PooledConnection conn = null; // 最终获取到的链接对象
            ...
            synchronized (state) {
                ...
                // 无空闲空闲连接
                    ...
                    if (state.activeConnections.size() < poolMaximumActiveConnections) {
                        ...
                        conn = new org.apache.ibatis.datasource.pooled.PooledConnection(dataSource.getConnection(), this);
                        if (log.isDebugEnabled()) {
                        ...
                        org.apache.ibatis.datasource.pooled.PooledConnection oldestActiveConnection = state.activeConnections.get(0);
                        // 检查该连接是否超时
                        ...
                        if (longestCheckoutTime > poolMaximumCheckoutTime) { // 检查到超时
                            ...
                            // 创建新的 PooledConnection 连接对象
                            conn = new org.apache.ibatis.datasource.pooled.PooledConnection(oldestActiveConnection.getRealConnection(), this);
    ...
    public static Connection unwrapConnection(Connection conn) {
        ...
        if (Proxy.isProxyClass(conn.getClass())) {
            ...
            if (handler instanceof org.apache.ibatis.datasource.pooled.PooledConnection) {
                return ((org.apache.ibatis.datasource.pooled.PooledConnection) handler).getRealConnection();
            }
        }
        return conn;
    }
